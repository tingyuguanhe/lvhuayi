<template>
  <div>
    ...
    <div>
      <el-button size="small" v-if="tableData.length < 20" type="primary" @click="addItem()">新建政务</el-button>
      ...
      <el-table :data="tableData" border v-loading="loading" style="width: 100%">
        ...
        <el-table-column label="功能状态" width="120">
          <template slot-scope="scope">{{scope.row.status | dataStatus}}</template>
        </el-table-column>
        <el-table-column prop="entryLink" label="功能地址"></el-table-column>
...
<script>
import {
  getGovermentAffairssList,
  createGovermentAffairss,
  updateWeight,
  delItem,
  updateStatus,
  updateGovernmentAffairss
} from "@/api/zhengwu";
    ...
    confirmCreate(formName) {
      this.$refs[formName].validate(async valid => {
        if (valid) {
          ...
          if (!this.id) {
            let res = await createGovermentAffairss(reqData);
            if (res.code == 200) {
            ...
            let res = await updateGovernmentAffairss(reqData);
            if (res.code == 200) {
