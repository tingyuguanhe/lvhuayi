<template>
  <div>
    ...
    <div>
      <el-button size="small" type="primary" @click="addItem()">新建指引</el-button>

      <el-table :data="tableData" border v-loading="loading" style="width: 100%">
        <el-table-column fixed prop="id" label="指引id" width="70"></el-table-column>
        <el-table-column label="创建时间">
          <template slot-scope="scope">{{scope.row.createTime | formatFullDate}}</template>
        </el-table-column>
        <el-table-column label="指引状态" width="120">
        ...
        <el-table-column prop="title" label="指引标题"></el-table-column>
        <el-table-column label="生效时间" width="120">
          <template slot-scope="scope">{{scope.row.startTime | formatFullDate}} 至 {{scope.row.endTime | formatFullDate}}</template>
        </el-table-column>
    ...
    <!-- 查看组件 -->
    <pre-view :dialogShow.sync="dialogShow"  :title="'指引'"  :currentRow="currentRow"></pre-view>
  </div>
</template>    
...
export default {
  ...
  methods: {
    ...
    async getListData() {
      ...
      let res = await getGuidesList(reqData);
      if (res.code == 200) {
    ...
    handleClickEdit(row) {
      this.$router.push({ name: "editGuide", params: { id: row.id } });
    },
