  ...
  <div>
    <div class="filter_wrap">
      <el-form
        ...
        class="demo-ruleForm"
      >
        <el-form-item label="瓷片标题">
          <el-input size="small" clearable v-model="ruleForm.title" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="瓷片状态">
          <el-select size="small" clearable v-model="ruleForm.status" placeholder="请选择瓷片状态">
            <el-option
              :label="item.label"
              :value="item.value"
    ...
    <div>
      <el-button size="small" type="primary" @click="addItem()">新建瓷片</el-button>

      <el-table :data="tableData" border v-loading="loading" style="width: 100%">
        <el-table-column fixed prop="id" label="瓷片id" width="70"></el-table-column>
        <el-table-column label="创建时间">
          <template slot-scope="scope">{{scope.row.createTime | formatFullDate}}</template>
        </el-table-column>
        <el-table-column label="瓷片状态" width="120">
        ...
        <el-table-column prop="title" label="瓷片标题"></el-table-column>
    ...
    <!-- 查看组件 -->
    <pre-view :dialogShow.sync="dialogShow" :title="'瓷片'" :currentRow="currentRow"></pre-view>
  </div>
</template>    
...
export default {
  ...
  methods: {
    ...
    addItem() {
      this.$router.push({ name: "createCipian" });
    },
    async getListData() {
      this.loading = true;
      this.tableData = [];
      ...
      let res = await getPorcelainList(reqData);
      if (res.code == 200) {
    ...
    handleClickEdit(row) {
      this.$router.push({ name: "editCipian", params: { id: row.id } });
    },
