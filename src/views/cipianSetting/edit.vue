      ...
      <el-col :span="15">
        <div class="grid-content bg-purple">
          <el-tabs v-model="activeName" @tab-click="handleClick(activeName)">
            <el-tab-pane label="英文瓷片" name="en"></el-tab-pane>
            <el-tab-pane label="中文瓷片" name="zh-CN"></el-tab-pane>
          </el-tabs>
          <!-- 英文瓷片 -->
          <el-form
            key="en"
          ...
          >
            <div v-show="activeName == 'en'">
              ...
              <el-form-item label="瓷片头图" prop="imageEng">
                <el-upload
                  key="en"
            ...
            <div v-show="activeName == 'zh-CN'">
              ...
              <el-form-item label="瓷片头图" prop="image">
                <el-upload
                  key="zhcn"
                  class="avatar-uploader"
                  :action="domain"
...
<script>
import * as CipianApi from "@/api/cipian";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        title: [{ required: true, message: "请输入标题", trigger: "blur" },{ min: 1, max: 20, message: '最多20个字符', trigger: 'blur' }],
  ...
  methods: {
    ...
    async getDetail() {
      let res = await CipianApi.getItemDetail(this.id);
      if (res.code == 200) {
        console.log(res);
        let Data = res.data;
    ...
    submitForm(formName) {
      this.$refs[formName].validate(async valid => {
        if (valid) {
          ...
          if (this.id) {
            //修改
            let res = await CipianApi.updatePorcelain(this.ruleForm);
            if (res.code == 200) {
            ...
            let res = await CipianApi.createPorcelain(this.ruleForm);
            if (res.code == 200) {
